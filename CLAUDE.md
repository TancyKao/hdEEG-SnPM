...
- **Verify everything:** `matlab -batch "test_all"` — one PASS/FAIL/SKIP summary ending in `VERIFY: ALL GREEN`: checkcode + headless GUI construct + `test_glm_snpm`/`test_lmm_snpm`/`test_legacy_snpm` (stat-correctness vs MATLAB builtins) + `test_event_group` (known XFAIL) + per-analysis output-file assertions + a synthetic spectral-folder smoke (`gen_synthetic_spectral_folder.m`) + edge/failure-mode tests. Excludes `test_core_snpm.m` (hardcoded paths). Per-analysis import/output/fixture reference: `docs/reference/ANALYSIS_CATALOG.md`. **Known gaps it flags:** circular analyses need the CircStat toolbox in `dependencies/` (currently missing → SKIP); the event-bridge `synthetic_events` `planted_truth` has a channel-index mismatch (XFAIL).
...
- Docs in `docs/` follow the Diátaxis framework (`docs/README.md` is the index): `tutorials/`, `how-to/`, `reference/` (backbone `reference/ANALYSIS_CATALOG.md`), `explanation/`, and `archive/` for frozen project history. Design/plan history — `PLAN.md`, `RECAP.md`, `MODIFICATIONS_SUMMARY.md`, `DESIGN_PROMPT_*.md` — now lives under `docs/archive/` (design prompts in `docs/archive/design-prompts/`); `docs/archive/MODIFICATIONS_SUMMARY.md` documents the v2.0 addition of Pearson correlation + covariate control (its line numbers have drifted from the current source — treat as history). Report HTML templates live in `templates/` (`anova_report_template.html`, `t_report_template.html`, `sleep_eeg_report.html`, `hdEEG_LMM.html`); `export_glm_report.m` resolves them via `fileparts(mfilename)/templates`.
